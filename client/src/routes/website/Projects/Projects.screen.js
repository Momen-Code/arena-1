...
const Projects = () => {
  ...
  const projectsTypes = PROJECTS.filter(
    (v, i, a) => a.findIndex((t) => t.type === v.type) === i
  ).map((project) => project.type);
  const location = useLocation();
  const [activeProjects, setActiveProjects] = useState(
    PROJECTS.slice(0, window.innerWidth > 576 ? 6 : 4)
  );
  const [inActiveProjects, setInActiveProjects] = useState(
    PROJECTS.slice(window.innerWidth > 576 ? 6 : 4)
  );
  ...
  useEffect(() => {
    if (selectedType == -1) {
      ...
      setActiveProjects(
        PROJECTS.filter(
          (project) => project.type == projectsTypes[selectedType]
        ).slice(0, activeProjects.length + (window.innerWidth > 576 ? 6 : 4))
      );
      setInActiveProjects(
        PROJECTS.filter(
          (project) => project.type == projectsTypes[selectedType]
        ).slice(activeProjects.length + (window.innerWidth > 576 ? 6 : 4))
      );
    }
  }, [selectedType]);
        ...
        <div className="bottom-wrapper">
          <div className="projects-grid">
            {activeProjects.map((project, i) => (
              <Link
                to={`${location.pathname}/${project.slug}`}
              ...
              >
                ...
                <h1 className="title">
                  {i18n.language == "ar" ? project.ar.title : project.en.title}
                </h1>
              </Link>
            ))}
          </div>
